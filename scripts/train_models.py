...
def main() -> None:
    ...
    for s in subjects:
        pos_path = features_dir / f"S{s:03d}_R{args.train_run:02d}_enroll.csv"
        pos_df = load_feature_frame(pos_path)
        pos_x = feature_matrix(pos_df)

        neg_arrays: list[np.ndarray] = []
        for j in subjects:
            if j == s:
                continue
            neg_path = features_dir / f"S{j:03d}_R{args.train_run:02d}_enroll.csv"
            neg_df = load_feature_frame(neg_path)
            neg_arrays.append(feature_matrix(neg_df))
